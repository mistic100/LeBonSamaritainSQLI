...
TwitterReader.prototype.updateCounters = function(tweets) {
    ...
    tweets.forEach(function(tweet) {
        if (tweet.created_at < that.startDate) {
            return;
        }
        ...
        if (!that.counters.helpers[tweet.target].helped[tweet.user.id]) {
            that.counters.helpers[tweet.target].helped[tweet.user.id] = 0;
        }
        that.counters.helpers[tweet.target].helped[tweet.user.id]++;
